...
// === Jadwal Pertemuan P2K2 (murni lokal di HP masing-masing pendamping) ===
// Sengaja TIDAK dikirim ke server — hanya untuk pengingat pribadi pendamping,
// jadi tidak butuh koneksi internet sama sekali untuk dibuat/dilihat/dihapus.
const JADWAL_PREFIX = 'jadwal_p2k2_';

export function loadJadwalList(nip) {
  if (typeof window === 'undefined') return [];
  return safeParse(localStorage.getItem(JADWAL_PREFIX + nip), []);
}

function saveJadwalList(nip, list) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(JADWAL_PREFIX + nip, JSON.stringify(list));
}

export function addJadwalLocal(nip, jadwal) {
  const list = loadJadwalList(nip);
  const item = { ...jadwal, ID: 'JP' + Date.now() };
  list.push(item);
  saveJadwalList(nip, list);
  return item;
}

export function deleteJadwalLocal(nip, id) {
  const list = loadJadwalList(nip).filter((j) => j.ID !== id);
  saveJadwalList(nip, list);
}
